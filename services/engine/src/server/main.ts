...
import {
  OAuthService,
  OAuthServiceLive,
} from '@foyer/core/oauth';
import { SecretService, SecretServiceLive } from '@foyer/core/secret';
    ...
    handlers
        ...
        Effect.gen(function* () {
          ...
          if (!row) {
            return yield* Effect.fail(
              new Error('Failed to create MCP'),
            );
          }
          return row;
        }),
      ...
      .handle('createSource', (request) =>
        Effect.gen(function* () {
          ...
          if (!row) {
            return yield* Effect.fail(
              new Error('Failed to create source'),
            );
          }
          return row;
        }),
      ...
      .handle('createTool', (request) =>
        Effect.gen(function* () {
          ...
          if (!row) {
            return yield* Effect.fail(
              new Error('Failed to create tool'),
            );
          }
          return row;
        }),
...
const OAuthApiLive = HttpApiBuilder.group(
  FoyerApi,
  'oauth',
  (handlers) =>
    handlers
